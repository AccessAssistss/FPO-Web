...
const FarmersInformation = () => {
    ...
    const [farmerBlock, setFarmerBlock] = useState('');  // Farmer block state for new farmer
    ...
    const handleSubmit = async () => {
        if (!farmerName || !farmerMobile || !farmerVillage || !farmerBlock) {
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                const newFarmer = {
                    id: response.results.id,  // Assuming the response contains the new farmer's ID
                    name: farmerName,
                    mobile: farmerMobile,
